...
class Backup
{
     ...
     * @return object 
     */
    public function setFile($file = null, $str="")
    {
        if(!empty($str)){
            $filename = $str.'-'.date('YmdHis');
        }
        else
        {
    ...
    public function import($table,$start)
    {
        //还原数据
        $db = self::connect();
     ...
     */
    public function backup($table, $start, $str='')
    {
        //初始化文件名
        $this->setFile(null,$str);
        $db = self::connect();
        // 备份表结构
        if (0 == $start) {
