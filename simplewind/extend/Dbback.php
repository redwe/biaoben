...
class DBbackup {
     ...
     */
    function __construct($host = 'localhost', $username = 'root', $password = '', $database = 'ltcs', $charset = 'utf8') {
        $this->host = $host;
        ...
        if (! empty ( $tablename )) {
            ...
            while ( $record = mysql_fetch_array ( $data ) ) {
                ...
                if (strlen ( $sql ) >= $size * 1024) {
                    ...
                    if ($this->_write_file ( $sql, $file, $dir )) {
                        $this->_showMsg("表-" . $tablename . "-卷-" . $p . "-数据备份完成<br>备份文件  " .$dir . $file ." ");
                    } else {
            ...
            if ($sql != "") {
                ...
                if ($this->_write_file ( $sql, $filename, $dir )) {
                    $this->_showMsg( "表-" . $tablename . "-卷-" . $p . "-数据备份完成<br>备份文件 " .$dir . $filename ." ");
                } else {
            ...
            while ( $table = mysql_fetch_array ( $tables ) ) {
                ...
                while ( $record = mysql_fetch_array ( $data ) ) {
                    ...
                    if (strlen ( $sql ) >= $size * 1000) {
                        ...
                        // 写入文件
                        if ($this->_write_file ( $sql, $file, $dir )) {
                            $this->_showMsg("卷-" . $p . "-数据备份完成<br>备份文件  ".$dir.$file." ");
                        } else {
            ...
            if ($sql != "") {
                $filename .= "_v" . $p . ".sql";
                if ($this->_write_file ( $sql, $filename, $dir )) {
                    $this->_showMsg("卷-" . $p . "-数据备份完成<br>备份文件  ".$dir.$filename." ");
                } else {
                    $this->_showMsg("卷-" . $p . "-备份失败",true);
                    return false;
                }
    ...
    private function _showMsg($msg,$err=false){
        ...
        echo "<p style='text-align: center'>".$err . $msg."</p>";
        flush();
    }


    public function _gotoback(){
